...
class GiveUpService(
    ...
    suspend fun offer(userId: Long, myToken: String, peerToken: String): GiveUpResult {
        ...
        //
        // Structural only — the bot-side, same pair, opposite sides, two different
        ...
        // A give-up is a bot-side concept, so the pairing must be on the bot-side —
        ...
        if (mine.chatId != NO_CHAT_ID || theirs.chatId != mine.chatId || theirs.pair != mine.pair ||
